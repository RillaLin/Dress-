...
<body>
	...
	<div id = "right">
		...
		<?php
		$msql = "UPDATE stores SET saccount = '{$_REQUEST['saccount']}' , spsd = '{$_REQUEST['spsd']}' , sname = '{$_REQUEST['sname']}' , semail = '{$_REQUEST['semail']}' , sphone = '{$_REQUEST['sphone']}' WHERE sid = {$_REQUEST['sid']}";
			$mresult = mysqli_query($conn,$msql);

			$rowUpdated = mysqli_affected_rows($conn);

			if($rowUpdated > 0){
				echo "<script>alert('成功更改店家資料~');location.href='root_store.php';</script>";
			}else{
				echo "<script>alert('更改失敗，請重新輸入');location.href='store_edit.php';</script>";
			}
		?>
	</div>
</body>
