...
if($_POST['psd'] != $_POST['againpsd']){
	$errMsg .= '您輸入的密碼不相同<br>';
}
elseif(!empty($_POST['psd']) || !empty($_POST['againpsd'])){
	$psd = $_POST['psd'];
	$sql = "SELECT * FROM members WHERE mpsd = '{$_POST['psd']}';";
}
else{
	$errMsg .= '您忘記輸入新密碼或再次輸入密碼<br>';
}
...
<body>
	...
	<div id="right">
		<?php
		    ...
		    $sql="SELECT * FROM members WHERE mid='{$_SESSION["mid"]}';";
			...
			if ($errMsg ==''){
				if($opwd==$_POST['opsd']){
					$strSQL = "UPDATE members SET mpsd='{$_POST['psd']}' WHERE mid='{$_SESSION["mid"]}';";
		...
		?>
 		<p><a href="memberpsd.php" style="background-color: #FFE4E1;color:#BC8F8F ">回修改密碼頁</a></p>
