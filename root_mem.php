<?php
	...
	if(!empty($_POST["srh"])){//關鍵字
       $sql="SELECT * FROM members WHERE mid={$_POST["srh"]}  ORDER BY mid ASC";
    }
    else{
       $sql = "SELECT * FROM members ORDER BY mid ASC";
    }
...
<body>
	...
	<div id = "right">
		...
		查詢會員id:<input type="text" name="srh"> &nbsp;&nbsp;
			...
			<?php
				...
				while($row = mysqli_fetch_array($result2)){
					echo "<tr>
						  ...
						  <td>$row[4]</td>
						  <td>$row[5]</td>
						  <td><a href='member_del.php?del={$row['mid']}'>刪除</a></td>
						  <td><a href='member_edit.php?del={$row['mid']}'>編輯</a></td>
						  </tr>";
				}
